...
class StringDBClient(BioDatabaseInterface):
    ...
    def response_to_strings(self, response: list[dict[str, Any]]) -> list[str]:
        result_strings = []
        for item in response:
            formatted_string_fields = [f"'{k}': '{v}'" for k, v in item.items()]
            formatted_string = "{" + ", ".join(formatted_string_fields) + "}"
